...
from codetalker.pgm.tokens import STRING, NUMBER, NEWLINE, WHITE, CharToken, StringToken
...
def t_dict(node):
    return dict((JSON.translate(key), JSON.translate(value)) for (key, value) in zip(node.keys, node.values))
